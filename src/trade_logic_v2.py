...
ES = pd.read_csv(r"../data/nasdaq.csv") # convert to raw string
NQ = pd.read_csv(r"../data/e-mini.csv")
...
ESvNQ = pd.merge(ES[['Date', 'Close']], NQ[['Date', 'Close']], on = 'Date', how = 'outer', suffixes = ('_ES', '_NQ'), sort = True)
ESvNQ.set_index('Date', drop = True, inplace = True) # set date to be index
ESvNQ.fillna(method = 'ffill', inplace = True)
...
def determine_position_1(entry): # determines first class's position today based on yesterday's position
    ...
    if previous_position == 0: # previous day was no position
        if entry['Ratio'] <= entry['Buy Enter']:
            ...
            pass
            print("determine_position_1: error1")
        ...
        if entry['Ratio'] < entry['Buy Exit']:
            ...
            print("determine_position_1: error2")
        ...
        if entry['Ratio'] <= entry['Buy Enter']:
            ...
            print("determine_position_1: error3")
    else:
        print("determine_position_1: error4")
        set_previous_pos(99)
        return 99
...
return_df_ESvNQ.to_csv("../data/pnl/NSvEQ.csv")